...
base_url="http://distfiles.gentoo.org/releases/arm/autobuilds"

latest_stage3=$(curl "${base_url}/latest-stage3-armv7a_hardfp.txt" 2>/dev/null | grep -v '#' | awk '{print $1}')
...
if [ ${xz} == true ] || [ ! -f stage3-armv7a_harfp.tar.xz ]; then
	echo "Transforming bz2 tarball to xz (golang bug). This will take some time..."
	bunzip2 -c "${stage3}" | xz -z > stage3-armv7a_hardfp.tar.xz || die "Failed to recompress to xz"
fi
